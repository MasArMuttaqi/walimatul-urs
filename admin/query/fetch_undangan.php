<?php
  ...
  if (in_array("kirim kabar", $undangan)) {
        $result_undangan = mysqli_query($conn, "SELECT agenda, tanggal, jam, tempat FROM agenda WHERE rec_num like '%Akad Nikah%'");   
        ...
        echo "Sehubungan dengan keterbatasan kapasitas yang kami miliki, kami belum dapat mengundang untuk turut hadir dalam acara tersebut. Mohon doa restu bagi Kami yang mengikat janji suci pernikahan tuk mengarungi bahtera rumah tangga bersama. Mohon maaf atas segala keterbatasan dan kekurangan kami.\r\n";
        ...
        foreach ($undangan as $value) {
         $result_undangan = mysqli_query($conn, "SELECT agenda, tanggal, jam, tempat FROM agenda WHERE rec_num='$value'");   
         $row_undangan = mysqli_fetch_assoc($result_undangan);
  	...
  	echo "_Wassalamu'alaikum wr wb_\r\nSalam,\r\nKami yang sedang berbahagia dan penuh syukur\r\nRiza dan Aftina\r\n Keluarga bapak H. Hardiyono dan Ibu Hj. Suciati beserta Keluarga Bapak (alm) Hardiyono dan Ibu Brantini Susilowati ";
mysqli_close($conn);
?>